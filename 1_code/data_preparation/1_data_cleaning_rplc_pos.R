...
sample_list <-
  readxl::read_xlsx("Stevenson Final List_pRPLC.xlsx",
                    sheet = 1,
                    col_types = "text")
...
object <-
  object %>%
  mutate(injection.order = as.numeric(injection.order))
...
plot <-
  ...
  activate_mass_dataset(what = "sample_info") %>%
  dplyr::mutate(batch = factor(batch, levels = stringr::str_sort(unique(batch), numeric = TRUE))) %>%
  log(10) %>%
  scale_data() %>%
  massqc::massqc_pca(color_by = "batch", point_alpha = 0.3)
